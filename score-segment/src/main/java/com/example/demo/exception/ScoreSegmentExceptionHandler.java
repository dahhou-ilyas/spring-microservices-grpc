...
public class ScoreSegmentExceptionHandler {
    ...
    public StatusRuntimeException handleValidationError(ScoreSegmentException e) {
        Instant time=Instant.now();

        Timestamp timestamp=Timestamp.newBuilder().setSeconds(time.getEpochSecond())
                .setNanos(time.getNano()).build();

        ScoreSegmentExceptionResponse exceptionResponse=ScoreSegmentExceptionResponse.newBuilder()
